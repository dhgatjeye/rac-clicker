use crate::core::{RacResult, ServerType, ClickPattern};
...
impl ServerRegistry {
    ...
    pub fn get_active(&self) -> RacResult<&ServerConfig> {
        self.configs
            ...
            .ok_or_else(|| crate::core::RacError::ConfigError(
                format!("Active server {:?} not found in registry", self.active_server)
            ))
    }

    pub fn get_active_mut(&mut self) -> RacResult<&mut ServerConfig> {
        self.configs
            .get_mut(&self.active_server)
            .ok_or_else(|| crate::core::RacError::ConfigError(
                format!("Active server {:?} not found in registry", self.active_server)
            ))
    }
    ...
    pub fn set_active(&mut self, server_type: ServerType) -> RacResult<()> {
        if !self.configs.contains_key(&server_type) {
            return Err(crate::core::RacError::ConfigError(
                format!("Server {:?} not found in registry", server_type)
            ));
        }
