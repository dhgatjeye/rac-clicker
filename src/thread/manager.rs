...
impl ThreadManager {
        ...
        F: FnOnce(Arc<ClickWorker>) + Send + 'static,
    {
        ...
        let worker = self.workers.get(&button)
            .ok_or_else(|| RacError::ThreadError(
                format!("Worker for {:?} button not registered", button)
            ))?;
        ...
        let handle = Builder::new()
            ...
            .map_err(|e| RacError::ThreadError(
                format!("Failed to spawn thread {}: {}", worker_name, e)
            ))?;
    ...
    pub fn start_signal(&self, button: MouseButton) -> RacResult<()> {
        let worker = self.workers.get(&button)
            .ok_or_else(|| RacError::ThreadError(
                format!("Worker for {:?} button not found", button)
            ))?;
