...
impl SettingsDisplayScreen {
    pub fn show(profile: &ConfigProfile) -> RacResult<()> {
        ...
        println!("═══ Hotkeys ═══");
        println!("  Toggle:        {}", HotkeyManager::key_name(profile.settings.toggle_hotkey));
        println!("  Left Click:    {}", HotkeyManager::key_name(profile.settings.left_hotkey));
        println!("  Right Click:   {}", HotkeyManager::key_name(profile.settings.right_hotkey));
        println!();
        ...
        println!("  Left:          {} {}", left_cps,
            if profile.settings.left_cps_override == 0 { "(server default)" } else { "(override)" });
        println!("  Right:         {} {}", right_cps,
            if profile.settings.right_cps_override == 0 { "(server default)" } else { "(override)" });
        println!();
