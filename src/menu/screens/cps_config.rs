...
impl CpsConfigScreen {
    ...
    fn configure_right_cps(profile: &mut ConfigProfile, settings_manager: &mut SettingsManager) -> RacResult<()> {
        let server = profile.server_registry.get_active()?;
    ...
    fn reset_to_defaults(profile: &mut ConfigProfile, settings_manager: &mut SettingsManager) -> RacResult<()> {
