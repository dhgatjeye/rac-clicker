...
impl HotkeyConfigScreen {
    ...
    fn configure_toggle_hotkey(profile: &mut ConfigProfile, settings_manager: &mut SettingsManager) -> RacResult<()> {
        ...
        if let Some(vk_code) = Self::wait_for_key_press()? {
            ...
            if vk_code == 0x2E {
                ...
                settings_manager.save(&profile.settings)?;
                println!("\n✓ Toggle hotkey set to: {}", HotkeyManager::key_name(vk_code));
                println!("✓ Settings saved!");
            }
        }
        Ok(())
    ...
    fn configure_left_hotkey(profile: &mut ConfigProfile, settings_manager: &mut SettingsManager) -> RacResult<()> {
        ...
        if let Some(vk_code) = Self::wait_for_key_press()? {
            if vk_code != 0x1B {
                ...
                settings_manager.save(&profile.settings)?;
                println!("\n✓ Left click hotkey set to: {}", HotkeyManager::key_name(vk_code));
                println!("✓ Settings saved!");
            }
        }
        Ok(())
    ...
    fn configure_right_hotkey(profile: &mut ConfigProfile, settings_manager: &mut SettingsManager) -> RacResult<()> {
        ...
        if let Some(vk_code) = Self::wait_for_key_press()? {
            if vk_code != 0x1B {
                ...
                settings_manager.save(&profile.settings)?;
                println!("\n✓ Right click hotkey set to: {}", HotkeyManager::key_name(vk_code));
                println!("✓ Settings saved!");
            }
        }
        Ok(())
    ...
    fn reset_all_hotkeys(profile: &mut ConfigProfile, settings_manager: &mut SettingsManager) -> RacResult<()> {
        profile.settings.toggle_hotkey = 0;
    ...
    fn wait_for_key_press() -> RacResult<Option<i32>> {
        ...
        for _ in 0..200 {
            ...
            for vk_code in [0x04, 0x05, 0x06] {  // MBUTTON, XBUTTON1, XBUTTON2
                unsafe {
